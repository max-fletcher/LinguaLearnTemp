...
export async function getUserCashBalanceHistories(
  req: AppAuthenticatedRequest,
  res: Response,
) {
  try {
    const oldestTransactionId = req.body.oldest_transaction_id
      ? req.body.oldest_transaction_id !== ''
        ? req.body.oldest_transaction_id
        : null
      : null;
    const limit = req.body.limit ? Number(req.body.limit) : 10;
    const cashBalanceHistoryData =
      await cashBalanceHistoryService.getUserCashBalanceHistoryInfScroll(
        req.user!.id,
        oldestTransactionId,
        limit,
      );
    ...
    const eduContents =
      await eduContentService.getEducationalContentByUniqueIds(
        eduContentToFetch,
        false,
        'id uniqueId category title price is_premium',
      );
    ...
    const formattedTransactions = formatCashBalanceHistory(
      cashBalanceHistoryData[1] as unknown as TransactionResponseType[],
      memoizedEduContents,
    );
    ...
    return res.json({
      data: {
        next:
          Number(cashBalanceHistoryData[0]) > 0 ? cashBalanceHistoryData[0] : 0,
